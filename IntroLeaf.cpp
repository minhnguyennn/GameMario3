...
CIntroLeaf::CIntroLeaf(float x, float y) :CGameObject(x, y)
{
	ax = 0;
	ay = LEAF_GRAVITY;
	isUp = false;
...
void CIntroLeaf::GetBoundingBox(float& l, float& t, float& r, float& b)
{
...
void CIntroLeaf::OnNoCollision(DWORD dt)
{
	x += vx * dt;
	y += vy * dt;
};

void CIntroLeaf::OnCollisionWith(LPCOLLISIONEVENT e)
{
...
bool CIntroLeaf::CountDownTimer(int time)
{
